...
if (Platform.OS === "web") {
  ({ doc, onSnapshot, runTransaction } = require("firebase/firestore"));
} else {
  ({ doc, onSnapshot, runTransaction } = require("@react-native-firebase/firestore"));
}
...
export default function BarberQueue() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        unsubscribe = onSnapshot(doc(firestore, "queue", data.uid), (snap:any) => {
          if (snap.exists()) {
  ...
  const handleRemove = (phoneToRemove: string, customerName: string) => {
    ...
    showActionToast({
      ...
      onConfirm: async () => {
        try {
          const qDocRef = doc(firestore, "queue", barberData.uid);
