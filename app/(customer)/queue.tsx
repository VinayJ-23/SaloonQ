...
if (Platform.OS === "web") {
  ({ doc, onSnapshot } = require("firebase/firestore"));
} else {
  ({ doc, onSnapshot } = require("@react-native-firebase/firestore"));
}
...
export default function CustomerQueue() {
    ...
    useCallback(() => {
      ...
      const loadInfo = async () => {
        try {
          ...
          const barberDocRef = doc(firestore, "barber", barberUID);;

          barberUnsubscribe = onSnapshot(barberDocRef, (barberSnap: any) => {
          ...
          const queueDocRef = doc(firestore, "queue", barberUID);
          unsubscribe = onSnapshot(queueDocRef, (snap:any) => {
            if (snap.exists()) {
  ...
  const confirmJoinQueue = async () => {
    ...
    try {
      const queueSnap = await getData("queue",barberPhone);
      console.log("done read at getdata 😂😂");
      ...
      if (queueSnap.exists()) {
        ...
        if (!alreadyExists) {
          await updateData("queue", barberPhone, newData);
        } else {
        ...
        await setData("queue", barberPhone,datax);
      }
      ...
      onConfirm: async () => {
        try {
          ...
          if (queueSnap.exists()) {
            ...
            const changeData={ customers: updatedCustomers }
            await updateData("queue", barberPhone, changeData);
