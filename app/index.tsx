...
import { BarberStorage } from '@/utils/BarberStorage.android';
...
export default function SplashScreen() {
  ...
  const checkAppVersion = async () => {
    try {
      ...
      const updateInfoSnap = await getData('admin', 'updateInfo');

      if (updateInfoSnap.exists()) {
        const data = updateInfoSnap.data() as UpdateInfo;
        const status = data.upToDate2_0_0;
